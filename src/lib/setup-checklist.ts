...
export const SETUP_ITEMS: SetupItem[] = [
  ...
  },
  {
    ...
    priority: 'quando_tiver',
    steps: [
      'Opção A (REST): pedir token ao suporte Avec (relatórios 0004, 0051, 0002) → AVEC_API_TOKEN',
      'Opção B (AvecLake): AVEC_LAKE_ACCESS_KEY_ID + AVEC_LAKE_SECRET_ACCESS_KEY (chaves AKIA… do e-mail AvecLake)',
      'AvecLake: AVEC_UNIT_ID = salao_id (40613 Brasil / 99801 Iguatemi) e AVEC_DATA_SOURCE=lake (ou auto)',
      'Opcional: AVEC_LAKE_REGION=us-west-2, AVEC_LAKE_WORKGROUP=avec_daas, AVEC_LAKE_DATABASE=avec_lake_db',
      'Remova AVEC_MOCK da Vercel (se existir) → Redeploy → Admin → Testar conexão → sync full',
    ],
    link: {
